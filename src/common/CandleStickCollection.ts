...
export class CandleStickCollection {
  ...
  getMissingCandles(expectedStart: Ticks, expectedEnd: Ticks) {
    expectedStart = baseUtils.toTimeEx(expectedStart).ceilToMinutes(this.minutesInInterval).ticks
    expectedEnd = baseUtils.toTimeEx(expectedEnd).floorToMinutes(this.minutesInInterval).ticks
