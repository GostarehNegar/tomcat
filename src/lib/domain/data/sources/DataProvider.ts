import { CandleStickCollection, Exchanges, ICandelStickData, Intervals, Markets, Symbols } from "../../base";
...
export class DataProvider implements IDataSource {
    ...
    constructor(exchange: Exchanges, market: Markets, symbol: Symbols, interval: Intervals) {
    ...
    async getData(startTime: number, endTime: number): Promise<CandleStickCollection> {
        ...
        if (result.length === 0 || result.endTime === endTime || result.startTime === startTime) {
