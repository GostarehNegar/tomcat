...
const CandleStreamWriter = tomcat.Index.Domain.Data.CandleStreamWriter
...
describe("pipeline", () => {
    test('first', async () => {
        ...
        const target = new CandleStreamWriter(myDataProvider, "test" + Math.floor(Math.random() * 100))
        ...
        const filterStream = new CandleStreamWriter(myDataProvider, pipeline.filters[0].name)
